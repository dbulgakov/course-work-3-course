...
class RedBlackTree:
    ...
    def print_tree(self, node, indent, last):
        # Prints the tree structure on the console
        if node != self.TNULL:
            print(indent, end='')
            if last:
                print("R----", end='')
    ...
    def search(self, key):
        result = self.__search_tree_helper(self.root, key)
        return None if result == self.TNULL else result
